...
then
code="A"
xvfb-run -a R --no-save --vanilla --slave --args $currentDir $dataDir $basefilename $futuresce $emission $period< $run_agmip_simple_delta
xvfb-run -a R --no-save --vanilla --slave --args $currentDir $basefilename $code $futuresce $emission $period< $run_agmip_simple2full
else
code="F"
xvfb-run -a R --no-save --vanilla --slave --args $currentDir $dataDir $basefilename $futuresce $emission $period< $run_agmip_simple_mandv
xvfb-run -a R --no-save --vanilla --slave --args $currentDir $basefilename $code $futuresce $emission $period< $run_agmip_simple2full
fi
done
