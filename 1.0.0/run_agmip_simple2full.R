...
for (scendecs in run.decs) {
  ...
  for (rcp in run.rcps) {
    ...
    for (thisgcm in run.gcms) {
      ...
      #filename    <- paste(substr(basefile,1,4), scencode[scendecs,rcp], gcmlist[thisgcm], 'XA', sep ='')
      ...
      agmip_simple2full(base, infile, outfile, headerplus,shortfile,stnlat,stnlon,stnelev,refht,wndht)
