...
deltloc     <- paste(dataDir, '/data/CMIP5/climfiles/', sep='')
latlonloc   <- paste(dataDir, '/data/CMIP5/latlon/', sep='')
...
#source('acr_findspot.R')
#source('agmip_simple_delta.R')

##  Define variables for function loop
baseinfo    <- read.table(paste(baseloc, basefile, '.AgMIP', sep=''), skip=3, nrows=1)
shortfile<-baseinfo$V1
stnlat<-baseinfo$V2
stnlon<-baseinfo$V3
stnelev<-baseinfo$V4

base<-paste(baseloc, basefile, '.AgMIP', sep='')
...
for (scendecs in run.decs) {
  ...
  for (rcp in run.rcps) {
    ...
    for (thisgcm in run.gcms) {
        ...
        #futname <- paste(shortfile,scencode[scendecs],gcmlist[thisgcm],'XA', sep ='')
      agmip_simple_delta(base,deltloc,latlonloc,futloc,futname,shortfile,stnlat,stnlon,stnelev,basedecs,futdecs,rcp,thisgcm)
      cat('\t', futname, ' created\n', sep='')
      flush.console()      
    }
  }
